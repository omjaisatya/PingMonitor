...
import { sendApiMonitorAlert } from "./emailService.js";
import { isMaintenanceActive } from "./maintenanceService.js";

export const dispatchApiNotifications = async (
  monitor,
  eventType,
  message,
  errorMsg = "",
  ...
  try {
    const isMaintenance = await isMaintenanceActive(monitor._id, "api");
    if (isMaintenance) {
      console.log(
        `Alert suppressed for api monitor ${monitor.name} due to active maintenance window.`,
      );
      return;
    }
    if (eventType === "down") {
      if (monitor.lastAlertedAt && monitor.alertCooldown) {
        ...
        if (elapsedMin < monitor.alertCooldown) {
          console.log(
            `API Deduplication: Skipping alert for ${monitor.name}. Cooldown active (${Math.round(elapsedMin)}/${monitor.alertCooldown}m).`,
          );
          return;
        }
      }
    ...
    if (delivery.webhook === "pending" && monitor.webhookUrl) {
      try {
        ...
        console.error(`API Webhook failed for ${monitor.name}:`, err.message);
    ...
    if (delivery.inApp === "pending") {
      try {
        ...
        console.error(`API In-App notification failed:`, err.message);
    ...
    if (delivery.email === "pending") {
      try {
        ...
        if (user && user.isVerified !== false) {
          ...
          await sendApiMonitorAlert({
            apiMonitorName: monitor.name,
            email: recipients,
    ...
    if (
      ...
      await ApiMonitor.findByIdAndUpdate(monitor._id, {
    ...
    console.error("API notifications runner failure:", err.message);
  }
};
